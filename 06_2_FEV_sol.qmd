...
`r log.model$coefficients[2] |> abs() |> (\(x) 2^x)() |> round(2)` larger for smoking
pupil than for non-smoking pupil
(95% CI [`r confint(log.model)[2,] |> (\(x) 2^x)() |> abs() |> round(2)`]).
...
`r ((log.model$coefficients[2] |> abs() |> (\(x) 2^x)() |> round(2))-1)*100`% larger for
smoking pupils than for non-smoking pupils
(95% CI [`r ((confint(log.model)[2,] |> (\(x) 2^x)() |> abs() |> round(2))-1)*100`]).
...
is  `r log.model$coefficients[1] |> (\(x) 2^x)() |> round(2)`l
(95% CI [`r confint(log.model)[1,] |> (\(x) 2^x)() |> abs() |> round(2)`]l).
