...
```{r nhanes, tidy=FALSE,echo=FALSE, cache=FALSE}
library(NHANES)
...
- Large sample
```{r}
qnorm(0.025, mean = xBar, sd = sdBar) |> (\(x) 2^x)()
qnorm(0.975, mean = xBar, sd = sdBar) |> (\(x) 2^x)()
2^(xBar - 2 * sdBar)
2^(xBar + 2 * sdBar)
```
...
- Small sample
```{r}
qnorm(0.025, mean = xBar10, sd = sdBar10) |> (\(x) 2^x)()
qnorm(0.975, mean = xBar10, sd = sdBar10) |> (\(x) 2^x)()
2^(xBar10 - 2 * sdBar10)
2^(xBar10 + 2 * sdBar10)
```
