...
```{r , out.width='100%', fig.asp=.8, fig.align='center',echo=FALSE, cache=FALSE}
library(GGally)
prostate |>
  select(-pgg45) |>
  ggpairs()
...
```{r out.width='80%', fig.asp=.8, fig.align='center', message=FALSE,echo=FALSE, cache=FALSE}
library(plot3D)
grid.lines <- 10
x <- prostate$lcavol
y <- prostate$lweight
...
```{r out.width='100%', fig.asp=.8, fig.align='center', message=FALSE,echo=FALSE, cache=FALSE}
par(mfrow = c(1, 2))
library(plot3D)
grid.lines <- 10
x <- prostate$lcavol
